...
const blank = { outwardNo: "", section: "", itemCode: "", itemName: "", colour: "", size: "", requiredPcs: 0, requiredMtr: 0, availableMtr: 0, issueType: "Material Shortage", reason: "", priority: "Medium", neededDate: "", remarks: "" };
...
export default function PendingPage({ notify }) {
  ...
  const columns = [
    ...
    {key:"actions",label:"Actions",render:(issue)=><div className="row-actions"><button onClick={()=>edit(issue)}>Edit</button><button onClick={()=>status(issue._id,"Material Requested")}>Request Store</button><button className="primary" onClick={()=>status(issue._id,"Resolved")}>Resolve</button></div>},
  ];
  return <><PageTitle title="Section Pending & Issues" subtitle="Colour shortage, rejection, production and sewing hold" />
    <Card title={editingId ? "Edit Pending Colour" : "Add Pending Colour Manually"}><form className="pending-form" onSubmit={submit}>{Object.keys(blank).map((key) => <label key={key}><span>{key}</span>{key === "issueType" ? <select value={form[key]} onChange={(event)=>setForm({...form,[key]:event.target.value})}>{["Material Shortage","Elastic Rejected","Production Hold","Rework Pending","Sewing Hold","Other"].map(value=><option key={value}>{value}</option>)}</select> : key === "priority" ? <select value={form[key]} onChange={(event)=>setForm({...form,[key]:event.target.value})}>{["Low","Medium","High","Critical"].map(value=><option key={value}>{value}</option>)}</select> : <input required={["section","colour","reason"].includes(key)} type={key === "neededDate" ? "date" : ["requiredPcs","requiredMtr","availableMtr"].includes(key) ? "number" : "text"} value={form[key] ?? ""} onChange={(event)=>setForm({...form,[key]:event.target.value})}/>}</label>)}<button className="primary">{editingId ? "Update Pending" : "Add Pending"}</button></form></Card>
