...
export default function ProductionControlPage({ notify }) {
  ...
  return <>
    ...
    <Card title="Start Production">
      <form className="production-start-grid" onSubmit={start}>{Object.keys(blankStart).filter(key=>!["dcNo","inwardNo"].includes(key)).map((key) => ["machineCode", "employeeCode"].includes(key) ? <QRScanInput key={key} field={key} label={key === "machineCode" ? "Machine QR" : "Employee QR"} value={startForm[key]} onChange={(scannedValue) => setStartForm((current) => ({ ...current, [key]: scannedValue.toUpperCase() }))} /> : <label key={key}><span>{key}</span>{key === "colour" && dcPlan ? <select required value={startForm.colour} onChange={(event)=>setStartForm({...startForm,colour:event.target.value})}><option value="">Select DC Colour</option>{dcPlan.colours.map(colour=><option key={colour}>{colour}</option>)}</select> : <input required={!['outwardNo'].includes(key)} readOnly={key === "outwardNo" && Boolean(startForm.outwardNo)} value={startForm[key]} type={key === "plannedPcs" ? "number" : "text"} onChange={(event) => setStartForm({ ...startForm, [key]: event.target.value })} />}</label>)}{startForm.inwardNo && <div className="scan-reference"><b>Source Inward</b><span>{startForm.inwardNo}</span></div>}<button className="primary">Start Production</button></form>
    </Card>
